defmodule SigninappEx.Domain.Model.Shared.Common.Validate.EmailTest do
  use ExUnit.Case
  ...
  describe "nil email" do
    test "rejects nil" do
      assert {:error, :email_empty} = Email.new(nil)
    end
  end
  ...
  describe "invalid type - non-binary inputs" do
    test "rejects atom" do
      assert {:error, :email_invalid_type} = Email.new(:user_at_example_dot_com)
    end

    test "rejects integer" do
      assert {:error, :email_invalid_type} = Email.new(123)
    end

    test "rejects float" do
      assert {:error, :email_invalid_type} = Email.new(12.34)
    end
    ...
    test "rejects map" do
      assert {:error, :email_invalid_type} = Email.new(%{"email" => "user@example.com"})
    end
    ...
    test "rejects boolean true" do
      assert {:error, :email_invalid_type} = Email.new(true)
    end

    test "rejects boolean false" do
      assert {:error, :email_invalid_type} = Email.new(false)
    end
  end
end
